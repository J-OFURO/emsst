...
c  17: CMC   (CMC)              
c  18: GMSSA (BOM)	        
c  19: MGDSST-RT (NEAR-GOOS)	        
c
c USAGE:
c  em_sst_v0.4.1  ISY IEY IEM
c
...
c  IASW(NP)  : switch of source products (actual,each grid)
c-----------------------------------------------------------------------  
      implicit none
      integer iargc,nargc
      external iargc
      character*4 argv
      integer iy,iyear,isy,iey,im,iem,id
      integer i,j,ix,jy
      integer icx,jcy
      integer n,NP
      parameter(NP=19)
...
c get argv
      ...
      do n=0,nargc
       ...
       if (n.eq.3) read(argv,'(i2)') iem
      enddo

c open data availability 
      OPEN(10,file="../ORG/AVAILABILITY/data_availability_V0.4.1.csv")
      read(10,*)
...
c----------
c EM
        fno1=path//'/DATA/DAY/V0.4.1/BIN/J-OFURO_EM_SST_V0.4.1_DAILY_'
     &           //year//'.bin'
        open(81,file=fno1,form="unformatted")

c NUM
        fno2=path//'/DATA/DAY/V0.4.1/BIN/J-OFURO_EM_NUM_V0.4.1_DAILY_'
     &           //year//'.bin'
        open(82,file=fno2,form="unformatted")

c AVE
        fno4=path//'/DATA/DAY/V0.4.1/BIN/J-OFURO_EM_AVE_V0.4.1_DAILY_'
     &           //year//'.bin'
        open(84,file=fno4,form="unformatted")
c STD
        fno5=path//'/DATA/DAY/V0.4.1/BIN/J-OFURO_EM_STD_V0.4.1_DAILY_'
     &           //year//'.bin'
        open(85,file=fno5,form="unformatted")
c MIN
        fno6=path//'/DATA/DAY/V0.4.1/BIN/J-OFURO_EM_MIN_V0.4.1_DAILY_'
     &           //year//'.bin'
        open(86,file=fno6,form="unformatted")

c MAX
        fno7=path//'/DATA/DAY/V0.4.1/BIN/J-OFURO_EM_MAX_V0.4.1_DAILY_'
     &           //year//'.bin'
        open(87,file=fno7,form="unformatted")

c EMS: List of IDs of source products
        fno8=path//'/DATA/DAY/V0.4.1/BIN/J-OFURO_EM_EMS_V0.4.1_DAILY_'
     &           //year//'.bin'
        open(88,file=fno8,form="unformatted")

c EMN: List of IDs of source products selected as median
        fno3=path//'/DATA/DAY/V0.4.1/BIN/J-OFURO_EM_EMN_V0.4.1_DAILY_'
     &           //year//'.bin'
...
c Patch for cutting tail data by iey-iem
           if (iy.eq.iey.and.im.gt.iem) then
            do i=1,ix
             do j=1,jy
              em(i,j)=spv
              nvd(i,j)=0
...
c MGDSST-RT
        fni="../ORG/ORG4/MGDSST_NEAR-GOOS_RT/DATA/BIN"
     &           //"/MGDSST_NEAR-GOOS_RT_SST_"
     &           //year//".bin"
        ENDIF
